...
const InflantionQuote = () => {
  ...
  async function handleGetQuoteByCurrency() {
    setIsLoading(true)
    try {
      const { data } = await api.get('/conversao/inflacao.php')

      setQuoteData(data.inflation[0])
    } catch (error) {
      toast.error(
  ...
  return (
    <>
      <Head>
        <title>Master Currency | Cotação da taxa de inflação do Brasil</title>
      </Head>
      <div className={styles.container}>
        <div className={styles.content}>
          ...
          <div className={styles.details}>
            <h1>Cotação da taxa de inflação do Brasil</h1>
...
export default InflantionQuote
